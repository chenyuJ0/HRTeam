...
<body>
	<!-- 网页顶部导航栏 -->
	<div class="x-nav">
		<span class="layui-breadcrumb"> <cite>文件信息</cite></a>
		</span> 
		<%-- <a class="layui-btn layui-btn-small"
			style="line-height: 1.6em; margin-top: 3px; float: innert; margin-left: 75%;"
			href="${ctx }/doc/add/" title="增加"> <i class="layui-icon"></i>增加
		</a>  --%>
		<a class="layui-btn layui-btn-small"
			style="line-height: 1.6em; margin-top: 3px; float: right"
			href="${ctx }/doc/showAll/" title="刷新"> <i class="layui-icon"
			style="line-height: 30px">ဂ</i></a>
	</div>
	<div class="x-body">
		<div class="layui-row" style="" align="center">
			<form class="layui-form layui-col-md12 x-so" method="post"
				action="${ctx }/doc/showAll">
				<table>
					<tr style="height: 50px">
						<td style="text-align: right; width: 120px"><span
							class="item_name" wit>文件名：</span></td>
						<td><input type="text" class="textbox textbox_225"
							style="width: 120px; height: 30px" name="filename"
							value="${document.filename}" /></td>
						<td style="width: 120px; text-align: right"><span
							class="item_name">文件备注：</span></td>
						<td><input type="text" class="textbox textbox_225"
							style="width: 120px; height: 30px" name="description"
		...
		<table class="layui-table">
			...
			<tbody>
				<c:forEach items="${info.list }" var="doc" varStatus="stat">
              ...
              <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'><i class="layui-icon">&#xe605;</i></div>
						...
						<%--  td>${doc.id}</td> --%>
						<td><a href="${ctx }/doc/download?id=${doc.id}">${doc.filename }</a></td>
						<td>${doc.description }</td>
						<td><fmt:formatDate value="${doc.create_date}"
								dateStyle="medium" /></td>
						<td>${doc.name }</td>
              ...
              <span class="layui-btn layui-btn-normal layui-btn-mini">已启用</span></td> -->
                ...
                <i class="layui-icon">&#xe601;</i>
							...
							<a title="编辑" href="${ctx}/doc/edit?id=${doc.id }"> <i
								class="layui-icon">&#xe642;</i>
						</a> <a title="删除" onclick="member_del(this,'${doc.id }')"
		...
		<nav aria-label="Page navigation">
			<ul class="pagination">
				...
				<c:if test="${info.pageNum ne 1 }">
					<li><a
						href="${pageContext.request.contextPath }/doc/showAll?pn=${info.pageNum -1 }&filename=${document.filename }&description=${document.description }"
				...
				<c:forEach begin="1" end="${info.pages }" step="1" var="i">
					...
					<c:if test="${info.pageNum ne i }">
						<li><a
							href="${pageContext.request.contextPath }/doc/showAll?pn=${i}&filename=${document.filename }&description=${document.description }">${i}</a>
						</li>
					</c:if>
				</c:forEach>
