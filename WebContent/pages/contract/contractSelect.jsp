			...
			<c:forEach items="${emp.contracts }" var="con">
				<tr>
					<td><a
						...
						<a href="ContractEdit.html">编辑</a>
						<a href="${pageContext.request.contextPath }/con/deleteCon?id=${emp.con.id}">删除</a></td>
					<td>${emp.jobnum }</td>
					<td>${emp.empname }</td>
					<td>${emp.dept.deptname }</td>
					<td>${emp.job.jobname }</td>
					<td><c:if test="${emp.work_status eq 0 }">在职</c:if> <c:if
							test="${emp.work_status eq 1 }">离职</c:if></td>
					<td><fmt:formatDate value="${con.contract_date }"
							dateStyle="medium" /></td>
					<td><fmt:formatDate value="${con.start_date }"
							dateStyle="medium" /></td>
					<td><fmt:formatDate value="${con.end_date }"
							dateStyle="medium" /></td>
					<td>${con.comment }</td>
			</c:forEach>
